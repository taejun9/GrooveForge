/**
 * 워크스테이션 UI 전반이 공유하는 선택 상태, 미리보기/결과 카드, 퀵 액션, 복구 데이터의 타입과 상수를 정의한다.
 * 도메인 모델과 렌더 분석을 화면 언어로 연결하는 계약 계층이며 실행 부수효과나 React 상태는 포함하지 않는다.
 * 저장 키·버전·크기 제한과 허용 id를 중앙화해 영속 데이터 역직렬화와 컴포넌트 간 계약 불일치를 줄인다.
 */
import type {
  ArrangementBlock,
...
export type PatternContrastRoleMapBlock = {
  index: number;
  sectionLabel: string;
  pattern: PatternSlot;
  role: PatternContrastRole;
...
export type PatternContrastRoleMapSummary = {
  statusLabel: string;
  headline: string;
  metricLabel: string;
  detailLabel: string;
