/**
 * 스타일 교체가 현재 프로젝트에 미칠 BPM·스윙·사운드·패턴 이벤트 변화를 미리 계산하고 실제 적용한다.
 * 미리보기와 적용 모두 같은 스타일 프로필/패턴 생성 규칙을 사용해 확인 화면과 결과가 어긋나지 않게 한다.
 * 알 수 없는 스타일 id는 미리보기 null 또는 원본 프로젝트 반환으로 닫아 기존 편집을 보존한다.
 */
import {
