...
Style Inspector work must derive read-only BPM range, active/default swing, bass role, melody role, sound preset, and Pattern A/B/C event density from local style profile and project data; keep style selection accessible/testable; preserve existing style application, undo/redo, save/load, playback, WAV/stem/MIDI export semantics; and avoid sampling, imported audio, hidden generation, remote AI, plugin hosting, accounts, analytics, or cloud sync.
