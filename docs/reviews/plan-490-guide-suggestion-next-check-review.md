# plan-490-guide-suggestion-next-check review

## Summary

Added a compact display-only after-run check line to the Quick Actions Guide Quick Start suggestion card so users know what to inspect immediately after running the suggested guide command.
...
- The suggestion after-run check line is derived from the current `guide-quick-start` Quick Action detail text and is display-only.
- Guide Quick Start target selection, source metadata, target metadata, metric metadata, reason line, pin-state metadata, Run behavior, and Pin/Unpin behavior are unchanged.
