# plan-489-guide-suggestion-metric review

## Summary

Added a compact display-only metric chip to the Quick Actions Guide Quick Start suggestion card so users can see the current readiness signal before running or pinning the command.
...
- The suggestion metric chip is derived from the current `guide-quick-start` Quick Action detail text and is display-only.
- Guide Quick Start target selection, source metadata, target metadata, reason line, pin-state metadata, Run behavior, and Pin/Unpin behavior are unchanged.
