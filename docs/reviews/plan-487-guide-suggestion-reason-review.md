# plan-487-guide-suggestion-reason review

## Summary

Added a compact display-only `Why now` reason line to the Quick Actions Guide Quick Start suggestion card so users can see the current session rationale before running or pinning the suggested guide command.

## QA

| command | result |
|---|---|
| `git diff --check` | passed |
| `python3 harness/scripts/run_qa.py` | passed |
| `python3 harness/scripts/run_quality_gate.py` | passed |
| `npm run typecheck` | passed |
| `npm run build` | passed with existing Vite large chunk warning |
| `npm run qa` | passed |
| `npm run verify` | passed with existing Vite large chunk warning; runtime smoke passed 14/14 blueprints and 14/14 style profiles |
| `npm run dev -- --host 127.0.0.1` | blocked by sandbox `listen EPERM` on `127.0.0.1:5173`; escalated retry was rejected by environment policy |

## Findings

- No blocking findings.
- The suggestion reason line is derived from the current `guide-quick-start` Quick Action detail text and is display-only.
- Guide Quick Start target selection, source metadata, pin-state metadata, Run behavior, and Pin/Unpin behavior are unchanged.
- The reason line is clamped to two lines in the compact suggestion card so the added rationale text does not resize or overlap the action controls.
- The change does not alter command execution, Quick Actions filtering, filtered order, Spotlight Enter target, Recent Commands, command ranking, project schema, undo/redo history, playback, save/load, render/export, Handoff, local draft, sampling, imported audio, remote AI, account, analytics, or cloud-sync behavior.

## Follow-Up

- Browser verification remains unavailable in this environment until local dev server binding is permitted.
