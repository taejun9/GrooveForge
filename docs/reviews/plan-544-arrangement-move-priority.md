# plan-544-arrangement-move-priority Review

## Summary

Added a read-only, UI-local Arrangement Move Priority Readout that derives the selected-block Drop, Build, Hook Lift, or Reset priority from existing selected arrangement block state and Arrangement Move preset definitions, showing action, reason, block scope, energy/mute impact, and next listening check before users apply a move.
