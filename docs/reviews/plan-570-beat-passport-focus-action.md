# plan-570-beat-passport-focus-action Review

## Summary

Added a visible action button to the Beat Passport Focus Readout. The button opens the current focused or highest-priority target, length, Pattern A/B/C use, readiness, export, stems, or master identity metric through the existing Beat Passport focus path, then preserves the existing Focus Result feedback and direct metric focus buttons.
