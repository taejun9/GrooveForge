...
- Storage: bounded versioned JSON project files with domain-owned import/serialization normalization, including a 64-bar total arrangement ceiling, 16-step note/chord end boundaries, 128-event note-track capacity, one chord per step, 16-event master-automation capacity, finite standard-MIDI pitch boundaries, and bounded finite sound/drum/note/chord/mixer/arrangement performance controls shared by project data, realtime scheduling, MIDI export, and audio conversion. The desktop main process owns a current-user-home `GrooveForge` workspace: `Projects` is the default portable-file location and `Data/grooveforge.db` is a SQLite3 schema-versioned catalog plus one bounded latest recovery mirror. Unknown wrapped project versions and newer SQLite schema versions fail closed rather than being resaved through an older schema.
...
First-run ownership state must describe the default event project as an editable 8-bar foundation rather than a demo. Before an explicit file save, the compact safety state remains warning-toned and must say the project is editable now, local only, and needs Save for a durable `.grooveforge.json` copy; the first mutation continues through the existing unsaved-change path, immediate renderer localStorage fallback, and debounced desktop SQLite recovery mirror. Neither recovery path may mark the project clean or claim an explicit Save.

The SQLite3 project library is an internal local index and recovery layer, not a second musical domain schema. It stores exact bounded `serializeProjectFile` JSON documents in STRICT tables through parameterized statements and short transactions; Pattern, event, mixer, Snapshot, and Session Brief structures remain owned by `ProjectState`. The renderer receives no SQL or database path capability, and recovery-save responses return only the save timestamp rather than echoing project JSON. Native Save writes and flushes the portable project file first, then mirrors that exact snapshot into SQLite; a mirror failure must report a warning without denying that the portable file was saved. Automatic recovery updates only the singleton recovery row and keeps the existing dirty, stale-completion, replacement, and close guards authoritative. Normal desktop execution is single-instance so two app processes cannot overwrite the shared singleton recovery row; isolated smoke processes use distinct injected workspaces.
...
Musical context identity keeps editable BPM and Key beside a read-only Time signature in the top setup row. The meter value comes from the domain-owned fixed `4/4` contract that matches the existing 16 sixteenth-step bar; `Fixed grid` copy prevents the readout from implying unsupported alternate-meter editing. This presentation adds no project field, file migration, scheduler branch, metronome accent, MIDI metadata, render-duration, or export-byte behavior.
...
Focused workspace pages are a second UI-only routing layer inside the persistent Compose and Mix functional tabpanels. Compose owns `drums`, `notes`, and `instruments`; Mix owns `mixer` and `master`. The active page ids and synchronous refs stay local to `App`, while each page panel remains mounted and is excluded from display and sequential focus with `hidden`. `WorkspacePageTabs` owns the reusable ARIA tablist, roving focus, ArrowLeft/ArrowRight/Home/End automatic activation, and nearest horizontal tab exposure. Central workspace routes first activate the outer functional tab, then the owning inner page, then resolve the live target for focus and scroll. Page-aware Delete and capture guards read the synchronous page ref so a selection left on a hidden editor cannot mutate project events. No page id enters project serialization, recovery, undo history, playback, rendering, export, or delivery data.
...
Handoff arrangement rows and Session Brief fields are generated from domain-normalized runtime views. Bars and energy use the same bounds as arrangement playback and serialization; brief whitespace collapses to single-line bounded fields before text output. Direct state therefore cannot make Handoff rows contradict audio/MIDI timing or inject section-like lines through a multiline brief.

Snapshot project cores are runtime boundaries as well as durable file data. Snapshot creation repairs the captured core through the same domain normalization used by serialization, restoration defensively repairs older or direct cores again, and summaries report repaired BPM, key, and bounded bars. A snapshot therefore cannot reintroduce UI state that contradicts its WAV, MIDI, Handoff, or saved JSON identity.

The focused Limiter ceiling range value is a transient editor draft, not independent project truth. One domain resolver converts the draft to the established ceiling range. Native Save/Open/playback commit it before reading `projectRef.current`; Undo/Redo cancel it before history restoration; and full-project, local-draft, or snapshot replacement rebase the editor to the replacement ceiling. This prevents render-closure state or a delayed blur from crossing a project lifecycle boundary.

Renderer-local recovery uses an explicit two-boolean write gate: replacement consumes one skip without writing the replacement itself, then the next armed project change writes normally. History restoration always arms that gate and marks the project dirty because Undo/Redo replaces editable project content; a later successful durable save remains the only conservative transition back to clean state without a saved-history cursor.

The Handoff rendered-WAV preview is a UI-local audition of the same deterministic offline full-mix Blob used by WAV export. It owns one transient `HTMLAudioElement` and one local object URL, records no export receipt, writes no file, and does not enter project data or undo history. Starting it stops realtime/editor audition; starting realtime playback or an individual editor audition stops it; project replacement or any project edit stops it; and stop, natural end, construction/playback failure, and unmount release the object URL so preview state cannot outlive the PCM snapshot it represents.

Project replacement reads dirty state from a synchronously maintained renderer ref rather than a potentially stale React render captured before a native/browser file dialog completes, and also reads the current parsed recovery object so a prior-session draft remains protected even when the starter itself is clean. The candidate file is parsed first, any focused Limiter ceiling draft is resolved into project truth, and only then does the shared load path resolve a dependency-free two-input replacement guard. Unsaved changes, a recovery draft, or both require a local discard confirmation before playback stop, history reset, project replacement, or local-draft clearing; cancellation returns before every destructive transition.

Asynchronous Save completion uses a monotonic renderer request id plus immutable project object identity. A completion whose id is not the latest is stale and cannot update file identity, result feedback, dirty state, or recovery state; full-project replacement advances that sequence before installing its own file identity. The latest successful native Save or browser download updates its file label, but only an exact `projectRef.current === projectToSave` match clears local recovery and marks clean. A mismatched identity means the durable file contains the captured snapshot while newer local edits remain dirty and recoverable; result feedback stays warning-toned until the user saves that newer snapshot.

Unsaved close protection is split at the renderer/Electron trust boundary. A dependency-free renderer decision contract maps clean, dirty, recovery-only, and combined states to confirmation and synchronous recovery-refresh requirements. `beforeunload` first commits any focused Limiter ceiling draft, writes the exact current dirty `projectRef`, and cancels unload; clean state returns without interference. Electron handles `will-prevent-unload` with a synchronous native warning whose Save default sends a validated `save-project-and-close` renderer command while leaving that unload blocked, whose cancel id keeps editing, and whose explicit close-without-project-file action alone calls Electron's inverted `preventDefault()` to allow unload. Before Save, a second dependency-free gate keeps recovery-only startup open and resurfaces Restore/Clear so the unrelated visible starter cannot replace the recovered project claim. Otherwise Save returns an explicit current/snapshot/stale/canceled/failed result; only current success clears synchronously mirrored dirty/recovery refs and uses the narrow preload close IPC to request another ordinary `BrowserWindow.close()`. The second close therefore passes through the same guard, while every non-current outcome remains open. Existing smoke processes exit through `app.exit` or process termination, so their noninteractive teardown does not enter this user decision path.
