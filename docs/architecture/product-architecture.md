...
- Storage: bounded versioned JSON project files with domain-owned import/serialization normalization, including a 64-bar total arrangement ceiling, 16-step note/chord end boundaries, 128-event note-track capacity, one chord per step, 16-event master-automation capacity, and finite standard-MIDI pitch boundaries shared by project data, realtime scheduling, MIDI export, and audio conversion, plus IndexedDB/local file cache in later plans. Unknown wrapped versions fail closed rather than being resaved through an older schema.
