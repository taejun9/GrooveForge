...
`npm run desktop:package-smoke` runs after build artifacts exist. On macOS it assembles an ignored `build/desktop/GrooveForge-darwin-*/GrooveForge.app` portable app from the installed Electron runtime, copies the built renderer plus Electron main/preload output into `Contents/Resources/app`, writes a minimal production `package.json`, validates app bundle naming and `Info.plist` privacy posture, then launches that packaged app with `GROOVEFORGE_DESKTOP_LAUNCH_SMOKE=1` to reuse the live beginner, producer, direct workstation, preload bridge, sampling-free copy, and screenshot pixel evidence checks. This smoke proves a local portable app bundle, not installer creation, code signing, notarization, auto-update, or distribution-channel QA.
