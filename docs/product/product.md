...
- Beat Blueprints that cover every supported style profile, then preview, apply, and show UI-local result feedback for complete sample-free project starts across style, key, BPM, Pattern A/B/C events, arrangement template, sound preset, mixer balance, and master preset while preserving manual editing.
