...
- Key Compass Focus: UI-local Focus controls that route scale, chords, 808/bass, melody, and selected-note or selected-chord diagnostics to the Compose panel so users can correct harmonic posture without changing key retargeting, note/chord editing, or project data; the Quick Actions Key Compass focus command reuses the same highest-priority key card and existing focus handler.
...
- Groove Compass Focus: UI-local Focus controls that route density, anchors, hat motion, timing, chance, and selected-drum diagnostics to the Compose panel so users can correct pocket posture without changing drum editing, selected drum state, playback, or project data; the Quick Actions Groove Compass focus command reuses the same highest-priority groove card and existing focus handler.
...
- First Beat Path: UI-local setup, compose, arrange, mix, and deliver progression derived from existing workflow/readiness/export signals so beginners know the next direct beat-making step and producers can scan session posture quickly; the Quick Actions First Beat Path jump command reuses the current next step and existing jump handler without changing path scoring or project data.
- Beat Spine: UI-local sample-free core readout for Setup, Drums, 808/Bass, Harmony, Melody, Sound, Arrange, and Finish derived from existing local project/readiness/export data, with explicit Jump controls and Quick Actions Beat Spine jump commands for panel navigation, explicit Apply controls and Quick Actions Beat Spine apply commands routed through existing undoable Layer Starter, Sound Preset, Pattern Chain, and Master Finish handlers, and a local Apply Result strip showing before/after metric, audition cue, and next check after user-clicked Apply actions.
...
- Arrangement view: editable pattern blocks placed into intro, verse, hook, bridge, and outro structures, with selected-block role readout, Arrangement Playback Readout for edit-versus-audible block context, Arrangement Playhead Highlighting across Arrangement Track, Section Locator Pads, and Song Form Overview from local playback snapshots, Arrangement Arc Pads with local Arrangement Arc Preview and Result for full-song energy shaping, Section Locator Pads for Intro/Verse/Hook/Bridge/Outro block cueing, Arrangement Focus presets with local Arrangement Focus Preview and Result plus Quick Actions Arrangement Focus access to the current selected-block suggestion, Pattern Chain 8-bar sketches with local Pattern Chain Preview and Result, per-step A/B/C chain cycling, Chain Expand song-form outlining, one-click Drop/Build/Hook Lift moves plus Quick Actions Arrangement Move access to the selected block's section-role suggestion, per-block bar lengths, selected-block copy/paste, split/merge controls, arrangement templates with local Arrangement Template Preview and Result plus duplicate, move, and delete controls.
...
- Production Snapshot Focus: UI-local Focus controls that route target, form, Pattern A/B/C coverage, mix, and handoff diagnostics to Compose, Arrange, Mix, or Deliver panels so users can move from session scan to the relevant workstation surface without changing snapshot scoring or project data; the Quick Actions Production Snapshot focus command reuses the current session scan metric and existing focus handler.
...
- Finish Checklist Focus: UI-local Focus controls that route Compose, Arrange, Mix, Master, and Handoff readiness cards to matching panels so users can run a guided finish pass without changing checklist scoring or project data; the Quick Actions Finish Checklist focus command reuses the current finish card and existing focus handler.
...
- Mixer/master: volume, pan, mute, solo, mixer channel role readouts, Stem Audition Pads for Full Mix/Drums/808/Synth/Chords solo checks plus Quick Actions Stem Audition commands, Mix Balance Pads with local Mix Balance Preview and Result for one-click editable rough balance plus a Quick Actions Mix Balance command for the current preview target, channel low-cut/air EQ, Drive/Glue mix controls, built-in Space send FX with Space FX Pads and Result plus Quick Actions Space FX pad commands, per-stem export level meters, deterministic Mix Coach checks, explicit Mix Fix actions with local Mix Fix Result feedback, Master Finish Pads with local Master Finish Preview and Result for editable demo/vocal/store/club output posture, master preset ceiling, master output role readout, limiter, loudness/peak metering.
- Mix Snapshot A/B: UI-local capture and comparison of two current mix/master passes by headroom, stem balance, master output, and audible stem posture from deterministic local export/stem analysis, with Quick Actions capture/clear commands, without changing project schema, playback, or export behavior.
...
- First Beat Path derived from existing local Project, StyleProfile, Workflow Navigator, Beat Map, Export Preflight, and export analysis data so users can scan setup, compose, arrange, mix, and deliver posture and jump to the existing workstation panels without mutating project data; the Quick Actions First Beat Path jump command exposes the same current next step from command search and routes only through the existing jump handler.
- Beat Spine derived from existing local Project, StyleProfile, Beat Readiness, arranged Pattern A/B/C event counts, Export Preflight, and export analysis data so users can scan Setup, Drums, 808/Bass, Harmony, Melody, Sound, Arrange, and Finish, jump to the matching panel, or explicitly apply the matching existing local beat-making action, then read the resulting before/after beat metric without introducing sampling entry points; Quick Actions Beat Spine jump/apply commands expose the same current card/action from the command palette and disable Apply when no Beat Spine action exists.
- Session Pass derived only from existing First Beat Path, Review Queue, Finish Checklist, Export Preflight, and project mode state, showing Guided, Studio, Finish, and Delivery focus cards that route to existing Transport, Compose, Arrange, Mix, Master, or Deliver panels without mutating project state, changing scoring, or running commands; the Quick Actions Session Pass focus command reuses that same explicit focus path for the current guided/studio pass card.
...
- Editable arrangement blocks and templates for 8-bar loop, full beat, hook-first, and breakdown structures, using assigned Pattern A/B/C data, selected-block role readout, Arrangement Playback Readout, Arrangement Playhead Highlighting across Arrangement Track, Section Locator Pads, Song Form Overview, Quick Actions Arrangement Block Jump/Cue commands for current block navigation and Block-loop audition, and local Arrangement Template Preview and Result, Quick Actions Arrangement Template access to the current suggested template, Arrangement Arc Pads with local Arrangement Arc Preview and Result for one-click full-song energy/section posture plus Quick Actions Arrangement Arc access to the current suggested arc, Section Locator Pads that cue the first Intro/Verse/Hook/Bridge/Outro block as the current Block loop without rewriting arrangement data plus Quick Actions Section Locator commands that reuse the same cue path, Arrangement Focus presets with local Arrangement Focus Preview and Result plus Quick Actions Arrangement Focus access to the current selected-block suggestion, one-click Drop/Build/Hook Lift moves plus Quick Actions Arrangement Move access to the selected block's section-role suggestion, per-block track mutes, per-block bar length, selected-block copy/paste, split/merge controls, audible per-block energy, and structure controls for duplicate, move, and delete.
...
- Desktop editing shortcuts, Keyboard Capture, and Quick Actions command search for arrangement playback, Arrangement Block Jump/Cue, Section Locator cueing, Pattern A/B/C selection, selected drum/note deletion, Keyboard Capture toggle, capture target, capture defaults, selected-drum edit tools, selected-note edit tools, selected-chord edit tools, MIDI Input Connect, MIDI Arm/Disarm, save, open, undo, redo, snapshots, blueprints, Delivery Target Align, Mode Focus jump, Beat Spine jump/apply, Key Retarget, Style Inspector focus, Style Quick Picks, Key Compass focus, Groove Compass focus, Pattern DNA focus, Layer Starter, Pattern Stack, Drum Move, 808 Move, Melody Move, Chord Move, Sound Preset, Drum Kit, Sound Focus, Listening Pass focus, Beat Passport focus, Production Snapshot focus, Finish Checklist focus, Composer Guide focus, Session Pass focus, Review Queue focus, Export Preflight focus, Workflow Spotlight focus, pattern fills, Pattern Chain/Chain Expand, Arrangement Template, Arrangement Arc, Arrangement Focus, Stem Audition, Mix Snapshot A/B, Mix Balance, Space FX, mix fixes, master finish, Handoff Next Export, and export while leaving focused inputs alone, then showing UI-only local result metrics, audition cues, and next checks after explicit command clicks.
...
- Mixer volume, pan, mute, solo, mixer channel role readouts, Stem Audition Pads that explicitly set Full Mix/Drums/808/Synth/Chords audition through existing mixer solo/mute state plus Quick Actions Stem Audition commands, Mix Balance Pads with local Mix Balance Preview and Result plus a Quick Actions Mix Balance command for the current preview target, low-cut/air EQ, Drive/Glue mix controls, built-in Space send FX with Space FX Pads and Result plus Quick Actions Space FX pad commands, per-stem export level meters, deterministic Mix Coach checks, explicit Headroom/Stem Balance/Low End Mix Fix actions with local Mix Fix Result, Master Finish Pads with local Master Finish Preview and Result, and master output role readout reflected in deterministic render feedback.
- Mix Snapshot A/B derives captured pass summaries and any Quick Actions Mix Snapshot capture/clear command targets only from current local project mixer/master state, deterministic export analysis, deterministic stem analysis, and UI-local slot state; the A/B slots stay UI-local and do not alter project files, undo history, playback, renders, or export contents.
...
Phase 5 adds mixer/master/export: volume, pan, mute, solo, mixer channel role readouts, Mix Balance Pads with local Mix Balance Preview and Result, Mix Snapshot A/B for UI-local pass comparison with Quick Actions capture/clear commands, channel low-cut/air EQ, Drive/Glue mix controls, built-in Space send FX with Space FX Pads and Result plus Quick Actions Space FX pad commands, per-stem export level meters, deterministic Mix Coach checks with local Mix Fix Result, Master Finish Pads with local Master Finish Preview and Result, master preset ceiling, master output role readout first, then limiter, peak/LUFS metering, WAV export, stem export, and MIDI export.
