...
- Editable arrangement blocks and templates for 8-bar loop, full beat, hook-first, and breakdown structures, using assigned Pattern A/B/C data, selected-block role readout, Arrangement Arc Pads for one-click full-song energy/section posture, Section Locator Pads that cue the first Intro/Verse/Hook/Bridge/Outro block as the current Block loop without rewriting arrangement data, Arrangement Focus presets, one-click Drop/Build/Hook Lift moves, per-block track mutes, per-block bar length, selected-block copy/paste, split/merge controls, audible per-block energy, and structure controls for duplicate, move, and delete.
