...
- Quick Actions: searchable local command palette for transport, project/session focus, creative, arrangement, mix, master finish, and export actions, with a UI-only post-run result strip that shows command status, one local metric, audition cue, and next check.
...
- Workflow Spotlight derived only from visible Workflow Navigator items so users can see the first blocker or review zone, exact jump target, and ready/review/blocker counts, then explicitly jump to the derived zone while preserving navigator item order, scoring, and project data.
