...
- Quick Actions: searchable local command palette for transport, project/session/workflow focus, creative, arrangement, mix, master finish, and export actions, with a UI-only post-run result strip that shows command status, one local metric, audition cue, and next check.
