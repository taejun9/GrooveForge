...
- Web MIDI Input adds explicit local MIDI keyboard/controller note capture for the selected 808 or Synth target, maps Note On messages to scale-locked Pattern A/B/C notes through existing Keyboard Capture defaults, exposes a Quick Actions MIDI Input Connect command through the existing local connect/refresh path, and never requests System Exclusive access or records audio.
