...
선택형 원샷 샘플링은 짧은 WAV를 드럼 레인에 연결하고 구간·음량을 조절하는 부가 기능입니다. 가져온 소리는 프로젝트와 함께 저장되고 재생·WAV 내보내기에 반영됩니다. 샘플 없이도 완전한 비트를 만들 수 있으며, 첫 화면과 기본 내비게이션은 계속 직접 작곡을 우선합니다.

화면 정보량을 줄이는 **간단 / 전체 도구** 전환과 프로젝트 간 재사용 가능한 **내 패턴 보관함**을 제공합니다. 자세한 사용법과 원샷 용량 제한은 [개인 작업 도구](docs/product/personal-tools.md)를 참고하세요.
...
- 현재 사용자 홈의 `GrooveForge/Projects`를 기본 위치로 사용하는 `.grooveforge.json` 저장과 열기
- `GrooveForge/Data/grooveforge.db` SQLite3 라이브러리와 자동 복구
- 브라우저 환경을 위한 기존 localStorage 복구 fallback
- 여섯 개 로컬 Snapshot
- Undo/Redo와 교체·닫기 보호
...
| `npm run desktop:project-io-smoke` | 네이티브 저장·열기와 SQLite3 catalog roundtrip |
