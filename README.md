...
- Local draft recovery that keeps a bounded browser/Electron renderer draft in localStorage, shows a project safety readout that distinguishes local draft safety from durable `.grooveforge.json` saves, then shows explicit Restore Draft and Clear Draft controls on the next session without replacing `.grooveforge.json` Save/Open.
